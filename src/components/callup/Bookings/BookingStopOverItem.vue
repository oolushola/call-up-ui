  ...
  <tr>
    ...
    <td>
      <span v-for="phoneNo in stop.parkId.contact.phoneNos" :key="phoneNo">
        <span class="d-inline-block bg-info rounded-pill badge me-1">{{ phoneNo }}</span>
      </span>
    </td>
    <td>{{ stop.parkId.contact.address }}</td>
    <td class="text-center">{{ timeConverter(stop.expectedDateOfArrival) }}</td>
    <td class="text-center">{{ timeConverter(dateCalculator(stop.expectedDateOfArrival, stop.stayingFor)) }}</td>
